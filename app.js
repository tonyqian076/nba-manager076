...
function makeProspects(season) {
  ...
  const available = [...basePool, ...trashPool].filter((seed) => !state.usedDraftNames.includes(seed[0]));
...
function submitContractOffer() {
  ...
  if (offer.type === "extension") {
    ...
    closeContractOffer();
    yourNewsLog(`${b.name} rejected your trade proposal and sent a follow-up.`);
    saveState();
    render();
    return;
  }
